...
# ==========================================
# Tieto údaje Streamlit ťahá z tvojho nastavenia v Streamlit Cloud (Secrets)
try:
        ...
        if not df_a.empty:
            ...
            if vs_col and pin_col:
                ...
                if not user_row.empty:
                    ...
                    st.rerun()
                else: st.error("Nesprávne VS alebo PIN kód.")
            else: st.error("V tabuľke 'Adresar' chýba stĺpec VS alebo PIN.")
    st.stop()

# --- DÔLEŽITÁ POISTKA PROTI CHYBE PRI ODHLÁSENÍ ---
...
try:
    ...
    with col_out2:
        if st.button("Odhlásiť sa", use_container_width=True):
            ...
            st.session_state["debt_confirmed"] = False
            st.rerun()
        ...
        if not df_p.empty:
            ...
            if stlpce_m:
                ...
                if not df_v.empty:
                    ...
                    fig = px.area(df_g, x="Mesiac", y="Zostatok", title="Vývoj financií (k dnešku)", template="plotly_dark")
    ...
    with tabs[2]:
        ...
        if not moje_riadky.empty:
            ...
            if bilancia < 0:
                st.markdown(f"""<div style="background-color:#fff5f5; padding:20px; border-radius:12px; border:3px solid #e53e3e; text-align:center;">
                    ...
                    <p>Suma všetkých predpisov: <b>{ocakavane:.2f} €</b> | Suma vašich úhrad: <b>{realne:.2f} €</b></p>
                ...
                st.markdown(f"""<div style="background-color:#f0fff4; padding:20px; border-radius:12px; border:3px solid #38a169; text-align:center;">
                    ...
                    <p>Suma predpisov: <b>{ocakavane:.2f} €</b> | Vaše úhrady: <b>{realne:.2f} €</b> | Preplatok: <b>{bilancia:.2f} €</b></p>
                </div>""", unsafe_allow_html=True)
        ...
        if je_zastupca_v_tabulke:
            ...
            for s_vs in sorted(susedia_vs):
                ...
                p_data.append({"VS": s_vs, "Stav": "Vyrovnané" if b_sus >= 0 else "Nedoplatok", "Suma (€)": f"{abs(b_sus):.2f}"})
    ...
    with tabs[3]:
        if OTAZKA.strip().upper() == "ŽIADNA":
            ...
            st.markdown(f"""<div style="background-color:#f0fff4; padding:15px; border-radius:10px; border:2px solid #38a169; margin-top:20px;">
                ...
                <p>Pošlite e-mail na adresu: <b>{MAIL_SPRAVCA}</b><br>
        ...
        if not df_h.empty:
            moje_h = df_h[df_h[vs_p].astype(str).str.replace(r'\.0$', '', regex=True).str.zfill(4) == u['vs']]
    ...
    with tabs[4]:
        ...
        st.markdown(f"""<div style="background-color:#f0f7ff; padding:15px; border-radius:10px; border:2px solid #007bff; margin-top:15px;">
            ...
            <p>Pošlite e-mail na <b>{MAIL_SPRAVCA}</b> s predmetom <b>ODKAZ NA NASTENKU | VS:{u['vs']}</b></p>
        </div>""", unsafe_allow_html=True)
        st.divider()
        if not df_o.empty:
