...
IBAN_FONDU = "SK0000000000000000000000" # SEM VLOŽTE REÁLNY IBAN PRE QR KÓD
...
try:
    ...
    with tabs[0]:
        if anketa_aktivna:
             st.markdown(f"""<div style="background-color:#fff3cd; padding:20px; border-radius:15px; border-left:8px solid #ffc107; margin-bottom:25px;">
                ...
                <p style="font-size:1.2em; font-weight:bold; color:#d9534f;">⏳ Koniec o: {zostava.days} dní ({koniec_dt.strftime('%d.%m.%Y')})</p>
        ...
        st.markdown(f"""
        <div style="background-color:#f8f9fa; padding:15px; border-radius:10px; border:1px solid #dee2e6; margin-top:15px;">
            <h4 style="color:#6c757d; margin-top:0;">📩 Manuálny návod</h4>
            <p style="color:#2d3748; font-size: 0.9em;">Pošlite e-mail na adresu: <b>{MAIL_SPRAVCA}</b><br>
    ...
    with tabs[1]:
        if not df_p.empty:
            ...
            if not df_v.empty and "Dátum" in df_v.columns:
                ...
                fig = px.area(df_g, x="Mesiac", y="Zostatok", title="Vývoj financií", template="plotly_white")
        ...
        if not moje_platby.empty:
            ...
            if bilancia < 0:
                ...
                st.markdown(f"""<div style="background-color:#fff5f5; padding:20px; border-radius:12px; border:3px solid #e53e3e; text-align:center;">
                    ...
                    <p style="color:#2d3748;">K mesiacu {t.month}/2026 má byť uhradených: <b>{ocakavane:.2f} €</b>.<br>
                    Vaše úhrady: <b>{realne:.2f} €</b>.</p>
                </div>""", unsafe_allow_html=True)
                ...
                st.image(buff.getvalue(), caption=f"QR Platba: {suma_str} € | VS: {u['vs']}", width=300)
            else:
    ...
    with tabs[3]:
        ...
        if not anketa_aktivna:
            st.error("⌛ Táto anketa už bola ukončená.")
        else:
