...
try:
    ...
    # --- ROZDELENIE DO KARIET (TABS) ---
    tab_nastenka, tab_financie, tab_uzivatel = st.tabs(["📢 Nástenka", "📊 Financie & Výdavky", "👤 Moja zóna & Hlasovanie"])

    # --- TAB 1: NÁSTENKA ---
    with tab_nastenka:
        st.subheader("Aktuálne oznamy pre obyvateľov")
        if not df_n.empty:
            # Najnovšie správy (zospodu tabuľky) idú navrch
    ...
    with tab_financie:
        ...
        with st.expander("📜 Zobraziť detailný zoznam výdavkov", expanded=False):
            if not df_v.empty:
            ...
            if not moje.empty:
                ...
                if OTAZKA.upper() != "ŽIADNA ANKETA":
                    ...
                    if not df_h.empty:
                        ...
                        # Ošetrenie názvu stĺpca (Hlas/HLAS)
                        ...
                        if not moj_h.empty:
                            ...
                            vys_text = "ÁNO 👍" if "ANO" in posledny else "NIE 👎"
                            st.warning(f"📢 **Váš zaevidovaný hlas:** {vys_text}")
...
st.write("---")
st.caption(f"© 2026 {NAZOV_PORTALU} | Dáta sa aktualizujú automaticky.")
