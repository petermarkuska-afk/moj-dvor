...
def get_df(sheet):
    try:
        ...
        df = pd.read_csv(url)
        df.columns = [str(c).strip() for c in df.columns] # Vyčistenie názvov stĺpcov
    ...
    if st.button("Prihlásiť sa"):
        ...
        if not df_a.empty and "VS" in df_a.columns:
            ...
            if not user_row.empty:
                ...
                st.error(f"VS {v_c_search} nebol v Adresári nájdený.")
        else:
            st.error("Chyba: Skontrolujte hárok 'Adresar' a stĺpec 'VS'.")
    st.stop()
...
try:
    ...
    t1, t2, t3, t4 = st.tabs(["📢 Nástenka", "📊 Financie", "💰 Moje platby", "🗳️ Anketa"])

    # --- NÁSTENKA A PODNETY ---
    with t1:
        st.markdown("### 📢 Aktuálne oznamy")
        if not df_n.empty: st.table(df_n.iloc[::-1])
        else: st.info("Žiadne nové oznamy.")
        
        st.divider()
        st.markdown("### 🛠️ Podnet pre správcu")
        msg = st.text_area("Napíšte váš podnet:", placeholder="Napr. Nefunkčná brána...")
        if msg:
            m_body = f"Odosielateľ: {user['meno']} (VS: {user['vs']})\nEmail: {user['email']}\n\nSpráva:\n{msg}"
            m_subj = f"Podnet - Victory Port - {user['meno']}"
            m_link = f"mailto:{MAIL_SPRAVCA}?subject={urllib.parse.quote(m_subj)}&body={urllib.parse.quote(m_body)}"
            st.link_button("🚀 Odoslať cez e-mailový klient", m_link, use_container_width=True)
        
        st.markdown(f"""
        > **Manuálne odoslanie:** Ak tlačidlo nefunguje, pošlite mail na **{MAIL_SPRAVCA}**. 
        > Do správy uveďte svoje meno (**{user['meno']}**) a VS (**{user['vs']}**).
        """)

    # --- FINANCIE (Opravený Graf) ---
    with t2:
        if not df_p.empty:
    ...
    # --- MOJE PLATBY (Opravené načítanie) ---
    with t3:
        st.markdown(f"### 💰 Tvoje platby (VS: {user['vs']})")
        if not df_p.empty and "VS" in df_p.columns:
            df_p["VS"] = df_p["VS"].astype(str).str.strip().str.zfill(4)
            # Dôležité: Filtrujeme presne podľa VS prihláseného usera
            moje = df_p[df_p["VS"] == user['vs']]
            if not moje.empty:
                # Zobrazíme len stĺpce s mesiacmi a VS
                st.dataframe(moje, hide_index=True, use_container_width=True)
            else:
                st.warning(f"V hárku 'Platby' sa nenašiel riadok s VS {user['vs']}. Skontrolujte, či je VS v oboch hárkoch rovnaký.")

    # --- ANKETA (Opravený layout) ---
    with t4:
        if OTAZKA.upper() != "ŽIADNA ANKETA":
            ...
            if not df_h.empty:
                ...
                # Formátovanie ZA / PROTI do dvoch pekných stĺpcov
                col_za, col_ni = st.columns(2)
                col_za.metric("AKTUÁLNE: ZA", f"{za} hlasov")
                col_ni.metric("AKTUÁLNE: PROTI", f"{ni} hlasov")

                v_c_clean = user['vs'].lstrip('0')
                moj_h = df_h[df_h.apply(lambda row: any(str(x).strip().lstrip('0') == v_c_clean for x in row), axis=1)]
                if not moj_h.empty:
                    st.warning(f"📢 Tvoj zaevidovaný hlas: {'ÁNO 👍' if 'ANO' in str(moj_h.iloc[-1][h_col]).upper() else 'NIE 👎'}")

            st.divider()
            s_za = f"HLAS_ANO_{user['vs']}: {OTAZKA}"
            s_ni = f"HLAS_NIE_{user['vs']}: {OTAZKA}"
            b1, b2 = st.columns(2)
            b1.link_button("👍 HLASUJEM ZA", f"mailto:{MAIL_SPRAVCA}?subject={urllib.parse.quote(s_za)}&body=Meno: {user['meno']}")
            b2.link_button("👎 HLASUJEM PROTI", f"mailto:{MAIL_SPRAVCA}?subject={urllib.parse.quote(s_ni)}&body=Meno: {user['meno']}")
        else:
