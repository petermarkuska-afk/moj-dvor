...
try:
    ...
    # --- T4: ANKETA (PRIPRAVENÁ PRE MAKE.COM) ---
    with tabs[3]:
        if OTAZKA.upper() != "ŽIADNA ANKETA":
            ...
            COL_HLAS = next((c for c in df_h.columns if "HLAS" in c.upper()), "Hlas")
            COL_OTAZKA = next((c for c in df_h.columns if "OTÁZKA" in c.upper() or "OTAZKA" in c.upper()), "Otázka")

            if not df_h.empty:
                # 1. Priebežné výsledky (len pre túto otázku)
                curr_q_df = df_h[df_h[COL_OTAZKA].astype(str) == OTAZKA] if COL_OTAZKA in df_h.columns else pd.DataFrame()
                
                za = len(curr_q_df[curr_q_df[COL_HLAS].astype(str).str.upper().str.contains("ANO|ÁNO")]) if not curr_q_df.empty else 0
                ni = len(curr_q_df[curr_q_df[COL_HLAS].astype(str).str.upper().str.contains("NIE")]) if not curr_q_df.empty else 0
                
                c1, c2 = st.columns(2)
                c1.metric("AKTUÁLNE ZA", f"{za} hlasov")
                c2.metric("AKTUÁLNE PROTI", f"{ni} hlasov")

                # 2. Kontrola, či užívateľ hlasoval v tejto téme (zablokuje tlačidlá)
                v_c_clean = u['vs'].lstrip('0')
                user_voted = curr_q_df[curr_q_df[COL_VS].astype(str).str.strip().str.lstrip('0') == v_c_clean] if not curr_q_df.empty else pd.DataFrame()
                uz_hlasoval = not user_voted.empty
            else:
                uz_hlasoval = False

            st.divider()
            if uz_hlasoval:
                st.success(f"✅ **Váš hlas bol k tejto otázke zaevidovaný.**")
                st.info("Hlasovanie k tejto téme je pre vás uzavreté.")
            else:
                st.write("### Odoslať váš hlas:")
                b1, b2 = st.columns(2)
                
                # Formát predmetu pre MAKE.COM: HLAS:VOLBA | VS:CISLO | TEXT OTAZKY
                subj_za = f"HLAS:ANO | VS:{u['vs']} | {OTAZKA}"
                subj_ni = f"HLAS:NIE | VS:{u['vs']} | {OTAZKA}"
                
                b1.link_button("👍 HLASUJEM ZA", f"mailto:{MAIL_SPRAVCA}?subject={urllib.parse.quote(subj_za)}&body=Meno: {u['meno']}", use_container_width=True)
                b2.link_button("👎 HLASUJEM PROTI", f"mailto:{MAIL_SPRAVCA}?subject={urllib.parse.quote(subj_ni)}&body=Meno: {u['meno']}", use_container_width=True)
                
                st.warning(f"**Manuálne hlasovanie:** Ak tlačidlá nefungujú, pošlite mail na **{MAIL_SPRAVCA}**. Predmet ZA: `{subj_za}`, Predmet PROTI: `{subj_ni}`.")

            # --- HISTÓRIA (Všetko, čo je v tabuľke pre dané VS) ---
            st.divider()
            ...
            if not df_h.empty:
                ...
                if not moje_vsetky.empty:
                    st.dataframe(moje_vsetky, hide_index=True, use_container_width=True)
                else:
