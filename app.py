...
try:
    ...
    # HLAVNÉ MENU (ZÁLOŽKY)
    tab_home, tab_fin, tab_user = st.tabs(["📢 Nástenka a Podnety", "📊 Financie areálu", "🔑 Moja zóna (VS)"])
    ...
    # --- ZÁOŽKA 3: MOJA ZÓNA (VS) ---
    with tab_user:
        st.markdown("### 🔑 Prístup k osobným údajom")
        vs_in = st.text_input("Zadajte váš VS (4 číslice) pre zobrazenie platieb a ankety:", placeholder="Napr. 0101")
            ...
            if not moje.empty:
                ...
                if OTAZKA.upper() != "ŽIADNA ANKETA":
                    ...
                    if not df_h.empty:
                        ...
                        if not moj_h.empty:
                            vysledok_ikona = "ÁNO 👍" if "ANO" in str(moj_h.iloc[-1][h_col]).upper() else "NIE 👎"
                    ...
                    t1, t2 = st.tabs(["Rýchle tlačidlá", "Manuálny návod"])
                    with t1:
                        b1, b2 = st.columns(2)
    ...
    st.error(f"Kritická chyba pri načítaní: {e}")
