...
NAZOV_PORTALU = "Victory Port" # Tu môžeš zmeniť názov portálu

st.set_page_config(page_title=NAZOV_PORTALU, layout="centered", page_icon="🏡")
...
if not st.session_state["authenticated"]:
    st.markdown(f"<h2 style='text-align: center;'>🔐 Vstup do {NAZOV_PORTALU}</h2>", unsafe_allow_html=True)
...
try:
    ...
    df_n = get_df("Nastenka") # Načítanie novej tabuľky
    ...
    # --- UI HLAVIČKA ---
    st.title(f"🏡 {NAZOV_PORTALU}")
    if st.button("Odhlásiť"):
        st.session_state["authenticated"] = False
        st.rerun()
    
    # --- MENU ---
    tab_nastenka, tab_financie, tab_uzivatel = st.tabs(["📢 Nástenka", "📊 Financie", "👤 Moja zóna & Hlasovanie"])

    # --- TAB 1: NÁSTENKA ---
    with tab_nastenka:
        st.subheader("Aktuálne oznamy")
        if not df_n.empty:
            # Otočenie poradia (najnovšie správy zospodu tabuľky pôjdu hore)
            for index, row in df_n.iloc[::-1].iterrows():
                with st.chat_message("user", avatar="📢"):
                    st.write(f"**{row['Dátum']} | {row['Autor']}**")
                    st.info(row['Odkaz'])
        else:
            st.info("Nástenka je momentálne prázdna.")
    ...
    # --- TAB 3: MOJA ZÓNA & HLASOVANIE ---
    with tab_uzivatel:
        vs_in = st.text_input("Zadajte váš VS (4 číslice) pre prístup k ankete a platbám:", key="vs_input")
            ...
            if not moje.empty:
                ...
                if OTAZKA.upper() != "ŽIADNA ANKETA":
                    ...
                    if not df_h.empty:
                        ...
                        # Ošetrenie, ak by stĺpec Hlas chýbal alebo bol inak pomenovaný (Make.com vs manuál)
                        hlas_col = "Hlas" if "Hlas" in df_h.columns else "HLAS"
                        df_h["Hlas_Upper"] = df_h[hlas_col].astype(str).str.upper()
...
st.write("---")
st.caption(f"© 2026 {NAZOV_PORTALU} | Dáta sú aktualizované v reálnom čase.")
